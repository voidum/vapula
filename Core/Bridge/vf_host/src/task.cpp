...
str TaskEx::GetCtrlConfig() { return _CtrlConfig; }

Dictionary* TaskEx::GetTags() { return _Tags; }

Task* TaskEx::Parse(strw path)
{
	str data = null;
	...
	str driver_id = xml::ValueA(xe_target->first_node("runtime"));
	...
	strw dir = xml::ValueW(xe_target->first_node("dir"));
	strw lib = xml::ValueW(xe_target->first_node("lib"));
	...
	while(xe_tag)
	{
		strw key = xml::ValueW(xe_tag->first_attribute("key"));
		strw value = xml::ValueW(xe_tag);
